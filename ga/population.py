...
class Population:
    ...
    def init_population(self, population_size=30):
        self.individuals = [Individual(self.available_weights, self.available_delays, self.gen_size)
                            for _ in range(population_size)]

    def get_bests(self, n=5):
        return sorted(self.individuals, reverse=True)[:n]

    def __repr__(self):
        return " ".join([str(individual) for individual in self.individuals])